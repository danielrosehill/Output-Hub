...
CREATE TABLE public.custom_gpts (
    ...
    is_gpt_optimised BOOLEAN NOT NULL DEFAULT FALSE,
    summary TEXT,
...
COMMENT ON TABLE public.prompt_outputs IS 'Stores outputs generated from prompts';
...
COMMENT ON COLUMN public.prompt_outputs.user_feedback IS 'User feedback on the output';
...
-- M2M relationship between custom GPTs and outputs
CREATE TABLE public.custom_gpts_outputs (
    custom_gpt_id INTEGER REFERENCES public.custom_gpts (id) ON DELETE CASCADE,
    output_id INTEGER REFERENCES public.prompt_outputs (id) ON DELETE CASCADE,
    PRIMARY KEY (custom_gpt_id, output_id)
);

-- M2M relationship between prompts and outputs
CREATE TABLE public.prompt_library_outputs (
    prompt_library_id INTEGER REFERENCES public.prompt_library (id) ON DELETE CASCADE,
    output_id INTEGER REFERENCES public.prompt_outputs (id) ON DELETE CASCADE,
    PRIMARY KEY (prompt_library_id, output_id)
);

-- GPT usage tracking
CREATE TABLE public.gpt_usage_tracking (
    id INTEGER PRIMARY KEY DEFAULT nextval('public.global_id_seq'),
    custom_gpt_id INTEGER NOT NULL REFERENCES public.custom_gpts (id) ON DELETE CASCADE,
    user_id INTEGER NOT NULL REFERENCES public.users (id) ON DELETE CASCADE,
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT current_timestamp,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT current_timestamp,
    UNIQUE (custom_gpt_id, user_id, usage_date)
);
...
CREATE TABLE public.user_favorites (
    ...
    CONSTRAINT check_favorite_type CHECK (
        (custom_gpt_id IS NOT NULL AND prompt_library_id IS NULL) OR
        (custom_gpt_id IS NULL AND prompt_library_id IS NOT NULL)
    )
);

-- Collaboration and sharing
CREATE TABLE public.collaboration_sharing (
    id INTEGER PRIMARY KEY DEFAULT nextval('public.global_id_seq'),
    owner_id INTEGER NOT NULL REFERENCES public.users (id) ON DELETE CASCADE,
    shared_with_id INTEGER NOT NULL REFERENCES public.users (id) ON DELETE CASCADE,
    custom_gpt_id INTEGER REFERENCES public.custom_gpts (id) ON DELETE CASCADE,
    prompt_library_id INTEGER REFERENCES public.prompt_library (id) ON DELETE CASCADE,
    permission_level VARCHAR(50) NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT current_timestamp,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT current_timestamp,
    CONSTRAINT check_shared_item_type CHECK (
        (custom_gpt_id IS NOT NULL AND prompt_library_id IS NULL) OR
        (custom_gpt_id IS NULL AND prompt_library_id IS NOT NULL)
    )
);
...
CREATE INDEX idx_custom_gpts_outputs_custom_gpt_id ON public.custom_gpts_outputs (custom_gpt_id);
CREATE INDEX idx_custom_gpts_outputs_output_id ON public.custom_gpts_outputs (output_id);
CREATE INDEX idx_prompt_library_outputs_prompt_library_id ON public.prompt_library_outputs (prompt_library_id);
CREATE INDEX idx_prompt_library_outputs_output_id ON public.prompt_library_outputs (output_id);
CREATE INDEX idx_gpt_usage_tracking_custom_gpt_id ON public.gpt_usage_tracking (custom_gpt_id);
CREATE INDEX idx_gpt_usage_tracking_user_id ON public.gpt_usage_tracking (user_id);
CREATE INDEX idx_gpt_usage_tracking_usage_date ON public.gpt_usage_tracking (usage_date);
CREATE INDEX idx_user_favorites_user_id ON public.user_favorites (user_id);
CREATE INDEX idx_user_favorites_custom_gpt_id ON public.user_favorites (custom_gpt_id);
CREATE INDEX idx_user_favorites_prompt_library_id ON public.user_favorites (prompt_library_id);
CREATE INDEX idx_collaboration_sharing_owner_id ON public.collaboration_sharing (owner_id);
CREATE INDEX idx_collaboration_sharing_shared_with_id ON public.collaboration_sharing (shared_with_id);
CREATE INDEX idx_collaboration_sharing_custom_gpt_id ON public.collaboration_sharing (custom_gpt_id);
CREATE INDEX idx_collaboration_sharing_prompt_library_id ON public.collaboration_sharing (prompt_library_id);
...
-- Add the requested M2M relationship between prompt outputs and prompts
CREATE TABLE public.prompt_outputs_prompts (
    prompt_output_id INTEGER REFERENCES public.prompt_outputs (id) ON DELETE CASCADE,
    prompt_id INTEGER REFERENCES public.prompt_library (id) ON DELETE CASCADE,
    PRIMARY KEY (prompt_output_id, prompt_id)
);

-- Add index for the new M2M relationship
CREATE INDEX idx_prompt_outputs_prompts_output_id ON public.prompt_outputs_prompts (prompt_output_id);
CREATE INDEX idx_prompt_outputs_prompts_prompt_id ON public.prompt_outputs_prompts (prompt_id);

-- Add JSON fields to main tables for additional flexibility
ALTER TABLE public.custom_gpts ADD COLUMN metadata JSONB;
ALTER TABLE public.prompt_library ADD COLUMN metadata JSONB;
ALTER TABLE public.prompt_outputs ADD COLUMN metadata JSONB;

-- Add GIN indexes for the new JSON fields
CREATE INDEX idx_custom_gpts_metadata ON public.custom_gpts USING GIN (metadata);
CREATE INDEX idx_prompt_library_metadata ON public.prompt_library USING GIN (metadata);
CREATE INDEX idx_prompt_outputs_metadata ON public.prompt_outputs USING GIN (metadata);

COMMENT ON COLUMN public.custom_gpts.metadata IS 'Flexible field for storing additional metadata about custom GPTs';
COMMENT ON COLUMN public.prompt_library.metadata IS 'Flexible field for storing additional metadata about prompts';
COMMENT ON COLUMN public.prompt_outputs.metadata IS 'Flexible field for storing additional metadata about outputs';